...
use organisation::poc::shared::{
    create_demo_client, demo_organisation_one,
...
async fn main() -> Result<()> {
    ...
    info!("Organisation {:?} connected to oracle smart contract: {} and peer_set_smart_contract: {}", 
        executing_organisation, oracle_sc.address(), peer_set_smart_contract.address()
    );

    info!("Listening to past events from peer set smart contract: {:?}", 
        peer_set_smart_contract.address()
    );
    peer_set_smart_contract.log_past_events().await?;

    info!("Listening for past validation request events from oracle: {:?}",
        oracle_sc.address()
    );
