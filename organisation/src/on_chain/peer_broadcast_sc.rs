...
pub trait PeerBroadcastService {
    async fn register_itself(&self, peer_ipfs_pointer: String) -> Result<()>;
...
impl PeerBroadcastService for EnrichedEthereumClient {
    async fn register_itself(&self, peer_ipfs_pointer: String) -> Result<()> {
        ...
        let call = peer_broadcast.register_peer(peer_ipfs_pointer);
    ...
    async fn register_peerset(
        ...
        let peer_set_smart_contract = self
            .deploy_peer_set_smart_contract(peer_set, initial_graph_ipfs_pointer)
            .await?;
