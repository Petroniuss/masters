...
pub struct EnrichedEthereumClient {
    ...
    pub permission_verifier_oracle:
...
impl ToEthereumClientEnriched for Arc<EthereumClient> {
    fn to_enriched_client(
        ...
        let ethereum_middleware =
            self.ethereum_middleware.clone();

        let executing_organisation =
            self.executing_organisation.clone();
        ...
        let permission_verifier_oracle =
            PermissionVerifierOracle::new(
                Address::from_str(
                    permission_verifier_oracle_address,
                )?,
                ethereum_middleware.clone(),
            );
...
pub fn crate_local_ethereum_client(
    ...
    let middleware =
        create_local_ethereum_middleware(provider, wallet)?;
